...
namespace TMG.StatBuffs
{
    public partial class PlayerControllerSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            Entities
                .WithAll<PlayerTag>()
                .ForEach((ref Translation translation, in TotalMoveSpeed moveSpeed, in TotalAttackDamage attackDamage) =>
                {
                    translation.Value.x += directionalInput * moveSpeed.Value * deltaTime;

                    if (attackInput)
                    {
                        ecb.AddComponent(enemy, new Damage{Value = attackDamage.Value});
                    }
                }).Run();
